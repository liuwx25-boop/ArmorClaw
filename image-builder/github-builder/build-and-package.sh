#!/usr/bin/env bash
# ============================================================
# ArmorClaw Full - One-click Build + Package (Global)
# ============================================================
# Run on build machine. Pipeline:
#   1. Build Docker image (calls build.sh: base + full layers)
#   2. Export image as armorclaw-{arch}.tar.gz
#   3. Copy to client/resources/
#   4. Package client installer (calls install_package.sh)
#
# Usage:
#   bash build-and-package.sh              # Full pipeline (current arch)
#   bash build-and-package.sh --arch all   # Package x64 + arm64 client installers
#
# Options:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            cat <<'EOF'
Usage: bash build-and-package.sh [options]

Full version: builds base + full image (with pre-installed skill tools), larger installer.
...
Image build (passed through to build.sh):
...
Examples:
  bash build-and-package.sh                          # Full pipeline
  bash build-and-package.sh --skip-build             # Image exists, export+package only
  bash build-and-package.sh --skip-package           # Build+export only, no client packaging
  bash build-and-package.sh --image-only             # Build+export+place in resources
  bash build-and-package.sh --platform mac --arch all
  bash build-and-package.sh --platform win --arch x64
EOF
            exit 0 ;;
...
if [ "$SKIP_PACKAGE" = false ] && [ -z "$CLIENT_PLATFORM" ]; then
    ...
    if [ "$HOST_OS" = "Linux" ]; then
        ...
        log_info "Example: bash build-and-package.sh --platform mac"
        log_info "         bash build-and-package.sh --platform win --arch x64"
        log_info "         bash build-and-package.sh --platform linux --arch x64"
        log_info "To build+export only, use --image-only"
        exit 1
    fi
fi
...
FULL_IMAGE="${IMAGE_NAME:-armorclaw:full}"
TAR_FILENAME="armorclaw-${BUILD_ARCH}.tar.gz"

echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  ArmorClaw Full - Build + Package${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if [ "$SKIP_BUILD" = true ]; then
    ...
    echo -e "    ${GREEN}●${NC} Image build (base + full)"
fi
...
if [ "$SKIP_BUILD" = false ]; then
    ...
    echo -e "${GREEN}[${CURRENT_STEP}/${TOTAL_STEPS}]${NC} Building Docker image (base + full, ${BUILD_ARCH})..."
    echo ""
    ...
    if ! docker image inspect "$FULL_IMAGE" &>/dev/null; then
        log_error "Image ${FULL_IMAGE} not found, build first or remove --skip-build"
        exit 1
    fi
    log_ok "Image confirmed: ${FULL_IMAGE}"
fi
...
if [ "$NO_IMAGE" = false ]; then
    ...
    IMAGE_SIZE=$(docker image inspect "$FULL_IMAGE" --format='{{.Size}}' | awk '{printf "%.1f", $1/1024/1024/1024}')
    log_info "Image virtual size: ${IMAGE_SIZE}GB, exporting+compressing (may take a few minutes)..."

    docker save "$FULL_IMAGE" | gzip > "$EXPORT_PATH"
...
if [ "$SKIP_BUILD" = false ] || docker image inspect "$FULL_IMAGE" &>/dev/null 2>&1; then
    FINAL_SIZE=$(docker image inspect "$FULL_IMAGE" --format='{{.Size}}' 2>/dev/null | awk '{printf "%.1f GB", $1/1024/1024/1024}')
    echo "  Docker image:  ${FULL_IMAGE} (${FINAL_SIZE})"
fi
