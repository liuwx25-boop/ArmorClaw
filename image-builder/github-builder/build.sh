#!/usr/bin/env bash
# ArmorClaw Image Builder - Full Build Script (Global)
# ============================================================
# Builds base + full image (with all skill tools pre-installed).
#
# Usage:
#   bash build.sh [options]
#   bash build.sh --local ../openclaw --skip-base
#   bash build.sh --push
# ============================================================
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --version)
            ...
            VERSION="$2"; shift 2 ;;
        -h|--help)
            echo "Usage: bash build.sh [--local PATH] [--skip-base] [--no-cache] [--verbose] [--push] [--browser] [--version VER]"
            echo ""
            echo "Options:"
...
if [ "$SKIP_BASE" = true ]; then
    ...
    # Windows Docker Desktop (BuildKit) COPY file-loss workaround
...
log_info "Building full image: $FULL_IMAGE"

BUILD_ARGS=()
[ -n "$NO_CACHE" ] && BUILD_ARGS+=("$NO_CACHE")
BUILD_ARGS+=(--build-arg "BASE_IMAGE=$BASE_IMAGE")
BUILD_ARGS+=(--build-arg "GOPROXY=${GOPROXY:-https://proxy.golang.org,direct}")
BUILD_ARGS+=(--build-arg "NPM_REGISTRY=${NPM_MIRRORS:-https://registry.npmjs.org}")
BUILD_ARGS+=(--build-arg "PIP_INDEX=${PIP_MIRRORS:-https://pypi.org/simple}")
docker build "${BUILD_ARGS[@]}" -t "$FULL_IMAGE" -f "$SCRIPT_DIR/Dockerfile.full" "$SCRIPT_DIR"
log_ok "Full image build complete"
...
VERSION_TAG="${FULL_IMAGE}-${SAFE_VERSION}-${BUILD_DATE}"
docker tag "$FULL_IMAGE" "$VERSION_TAG" 2>/dev/null && log_info "Version tag: $VERSION_TAG" || true

# ============================================================
# Verify pre-installed tools
# Tool list maintenance: update this list when modifying Dockerfile.full
# ============================================================

log_info "Verifying pre-installed tools..."
TOOLS=(
    # APT
    op
    # Node CLIs
    claude obsidian clawhub mcporter summarize oracle gog gemini
    # Go CLIs
    blu camsnap eightctl goplaces ordercli sonos songsee wacli blogwatcher
...
OK=0; FAIL=0
for t in "${TOOLS[@]}"; do
    if docker run --rm "$FULL_IMAGE" sh -c "which $t" &>/dev/null; then
        OK=$((OK + 1))
...
echo "Size:  $(docker image inspect "$FULL_IMAGE" --format='{{.Size}}' | awk '{printf "%.1f GB", $1/1024/1024/1024}')"
echo "Tools: $OK/${#TOOLS[@]} passed, $FAIL/${#TOOLS[@]} failed"
echo "=================================="
...
if [ "$PUSH" = true ]; then
    ...
    log_info "Pushing image to: $REGISTRY_IMAGE"
    docker tag "$FULL_IMAGE" "$REGISTRY_IMAGE:latest"
    docker tag "$FULL_IMAGE" "$REGISTRY_IMAGE:${SAFE_VERSION}-${BUILD_DATE}"
    docker push "$REGISTRY_IMAGE:latest" || { log_error "Failed to push latest tag"; exit 1; }
    docker push "$REGISTRY_IMAGE:${SAFE_VERSION}-${BUILD_DATE}" || { log_error "Failed to push version tag"; exit 1; }
    log_ok "Image pushed: $REGISTRY_IMAGE"
fi
